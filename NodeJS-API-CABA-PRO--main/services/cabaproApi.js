...
function normalizeBaseUrl(url) {
    ...
    // If user provided only host or IP (e.g. 3.237.233.7 or 3.237.233.7:9000),
    // prepend http:// so URL parsing works.
    ...
    try {
        ...
        // Keep port only if explicitly provided by the user/URL
        const port = u.port;

        // Normalize path: remove trailing slash and a final '/api' if present
        let pathname = (u.pathname || "").replace(/\/+$/g, "");
        if (pathname.toLowerCase().endsWith("/api")) {
            pathname = pathname.slice(0, -4).replace(/\/+$/g, "");
        }

        const hostPart = `${u.hostname}${port ? ":" + port : ""}`;
        const pathPart = pathname ? `/${pathname}` : "";
        let normalized = `${u.protocol}//${hostPart}${pathPart}`;
